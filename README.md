...
If you are running the API from online services (like Onyxia), make sure you can access the port 5000. If the url is not `http://localhost:5000/api`, make sure to modify the content of the following files:

- `frontend/.env`: change VITE_API_URL with the url generated by the web service for the port 5000
- `frontend/vite.config.ts`: add the same url to `allowedHosts`, alternatively set the parameter to True instead of the full list
